...
public class IdEqualsOrAdminInterceptor implements HttpInterceptor {
    ...
    private String getToken(final String tokenHeader) {
        ...
        if (tokenHeader.contains("Bearer")) {
            return Arrays.stream(tokenHeader.split(" ")).toList().getLast();
        }

        return tokenHeader;
    }

    private boolean isIdEqualsOrAdmin(final UUID userId, final UUID idFromToken, final UserRole userRoleFromToken) {
